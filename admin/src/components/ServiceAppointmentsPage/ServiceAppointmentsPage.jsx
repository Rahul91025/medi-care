...
export default function ServiceAppointmentsPage() {
  ...
  async function fetchAppointments() {
    try {
      const res = await axios.get(`${API_BASE}/api/service-appointments`);
  ...
  async function updateStatus(id, newStatus) {
    try {
      await axios.put(`${API_BASE}/api/service-appointments/${id}`, { status: newStatus });
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[11px] font-semibold text-emerald-600 tracking-wide uppercase">Bookings</span>
        </div>
        <h1 className="text-2xl font-heading font-extrabold text-slate-900 tracking-tight">Service Appointments</h1>
        <p className="text-sm text-slate-500 mt-1">Track and manage all service bookings.</p>
      </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead>
              <tr className="border-b border-slate-100">
                ...
                <th className="px-6 py-3.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Service</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {appointments.map((a) => (
                <tr key={a._id || a.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-medium text-slate-800 text-[13px]">{a.serviceName}</div>
                  </td>
                  <td className="px-6 py-4 text-slate-600 text-[13px]">
                    <div className="font-medium">{a.date}</div>
                    <div className="text-[11px] text-slate-400 mt-0.5">
                      {a.hour !== undefined ? `${a.hour}:${String(a.minute || 0).padStart(2, "0")} ${a.ampm || ""}` : ""}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    {a.status !== "Completed" && a.status !== "Canceled" && (
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => updateStatus(a._id || a.id, "Completed")} className="p-1.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-lg transition-colors" title="Complete">
                          <CheckCircle size={15} />
                        </button>
                        <button onClick={() => updateStatus(a._id || a.id, "Canceled")} className="p-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors" title="Cancel">
                          <XCircle size={15} />
                        </button>
                      </div>
                    )}
              ...
              {appointments.length === 0 && (
                <tr>
                  <td colSpan="6" className="px-6 py-16 text-center text-slate-400 text-sm">No service appointments found.</td>
                </tr>
              )}
            </tbody>
          </table>
