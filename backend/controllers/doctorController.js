...
function normalizeDocForClient(raw = {}) {
  ...
  if (doc.schedule && typeof doc.schedule.forEach === "function") {
    const obj = {};
    doc.schedule.forEach((val, key) => { obj[key] = Array.isArray(val) ? val : []; });
    doc.schedule = obj;
...
export const loginDoctor = async (req, res) => {
  try {
    ...
    delete out.password;
    return res.json({ success: true, data: out, token: doctor._id.toString() });
  } catch (err) {
...
export const getDoctorById = async (req, res) => {
  try {
    ...
    const out = normalizeDocForClient(doctor);
    delete out.password;
...
export const updateDoctor = async (req, res) => {
  try {
    ...
    if (body.password) existing.password = await bcrypt.hash(body.password, 10);
...
export const deleteDoctor = async (req, res) => {
  try {
    ...
    if (doctor.imagePublicId) {
      deleteFromCloudinary(doctor.imagePublicId).catch(() => { });
    }
