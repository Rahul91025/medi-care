...
export const serviceDetailStyles = {
  // Page container
  pageContainer: "min-h-screen font-sans bg-white px-4 lg:px-12 pt-20 sm:pt-12 md:pt-8 lg:pt-0",

  // Navigation bar
  navBar: "backdrop-blur-xl top-0 z-20",
  navContainer: "max-w-6xl mx-auto h-16 flex items-center justify-between px-4",
  backButton: "inline-flex items-center gap-2 px-5 py-2 bg-slate-50 text-slate-700 border border-slate-200 rounded-full hover:bg-slate-100 transition-all font-medium",
  ...
  // Image
  imageContainer: "w-full h-56 sm:h-72 md:h-96 lg:h-[65vh] xl:h-[70vh] rounded-[2.5rem] overflow-hidden shadow-xl border border-slate-100",
  image: "w-full h-full object-cover object-center transition-transform duration-500 hover:scale-105",

  // Details form
  detailsContainer: "bg-white p-8 rounded-[2rem] shadow-lg border border-slate-100",
  detailsTitle: "text-lg font-heading font-bold text-slate-900 flex items-center gap-3 mb-6",
  detailsGrid: "grid grid-cols-1 sm:grid-cols-2 gap-4",

  // Input fields
  input: "px-5 py-4 rounded-2xl border border-slate-200 bg-slate-50 w-full text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 focus:bg-white transition-all placeholder:text-slate-400",
  invalidInput: "px-5 py-4 rounded-2xl border-2 border-rose-300 bg-rose-50 w-full text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-rose-400 transition-all",
  emailInput: "px-5 py-4 rounded-2xl border border-slate-200 bg-slate-50 w-full sm:col-span-2 text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 focus:bg-white transition-all placeholder:text-slate-400",

  // Payment method
  paymentLabel: "font-heading font-bold text-slate-900 block mb-3",
  paymentOptions: "inline-flex gap-3",
  paymentOption: (isSelected) =>
    `px-5 py-2 rounded-full cursor-pointer border-2 font-medium transition-all duration-300 ${isSelected ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-700 border-slate-200 hover:border-primary-400"}`,
  paymentInput: "hidden",

  // Date selection
  dateSection: "mt-6",
  dateTitle: "text-xl font-heading font-bold text-slate-900 mb-3",
  ...
  dateButton: (isSelected) =>
    `px-5 py-3 rounded-2xl cursor-pointer border-2 transition-all duration-300 whitespace-nowrap min-w-[140px] sm:min-w-0 font-medium ${isSelected ? "bg-slate-900 border-slate-900 text-white shadow-lg shadow-slate-900/20" : "bg-white border-slate-200 text-slate-700 hover:border-primary-400 hover:bg-primary-50"}`,

  // Time selection
  timeSection: "mt-6",
  timeTitle: "text-xl font-heading font-bold text-slate-900 mb-3",
  ...
  timeButton: (isSelected) =>
    `px-5 py-3 rounded-2xl cursor-pointer border-2 transition-all duration-300 whitespace-nowrap min-w-[140px] sm:min-w-0 flex items-center gap-2 font-medium ${isSelected ? "bg-slate-900 border-slate-900 text-white shadow-lg shadow-slate-900/20" : "bg-white border-slate-200 text-slate-700 hover:border-primary-400 hover:bg-primary-50"}`,
  noSlotsMessage: "text-slate-400 p-2 font-medium",

  // Submit button
  errorMessage: "text-rose-500 mb-3 font-medium",
  successMessage: "text-primary-600 mb-3 font-medium",
  submitButton: (isValid, isSubmitting) =>
    `w-full py-4 md:mb-8 rounded-2xl cursor-pointer text-lg font-semibold flex items-center justify-center gap-3 transition-all duration-300 ${isValid && !isSubmitting ? "bg-slate-900 text-white shadow-lg shadow-slate-900/20 hover:bg-primary-600 hover:shadow-primary-500/25 hover:-translate-y-0.5" : "bg-slate-100 text-slate-400 cursor-not-allowed"}`,

  // Right column
  rightColumn: "bg-white rounded-[2.5rem] shadow-xl p-8 sm:p-10 border border-slate-100 h-fit",
  serviceName: "text-3xl lg:text-4xl xl:text-4xl md:text-3xl sm:text-4xl font-heading font-extrabold text-slate-900",

  // About section
  aboutContainer: "mt-8 bg-slate-50 p-6 rounded-2xl border border-slate-100",
  aboutTitle: "flex items-center gap-3 text-lg md:text-xl font-heading font-bold text-slate-900",
  aboutText: "text-slate-600 mt-3 leading-relaxed",

  // Price display
  priceContainer: "mt-6 rounded-2xl flex items-center gap-3 bg-slate-50 w-fit px-6 py-4 border border-slate-100",
  priceText: "font-bold text-2xl text-slate-900",

  // Instructions
  instructionsContainer: "mt-8",
  instructionsTitle: "text-xl font-heading font-bold text-slate-900 mb-4",
  instructionsList: "list-disc pl-6 text-slate-600 space-y-2",

  // Booking summary
  summaryContainer: "mt-8 bg-slate-50 rounded-2xl p-6 border border-slate-100",
  summaryTitle: "text-lg font-heading font-bold text-slate-900 mb-5",
  summaryContent: "space-y-3 text-slate-600 text-sm sm:text-base",
  summaryItem: "",

  // Loading and error states
  loadingContainer: "min-h-screen flex items-center justify-center p-8",
  loadingCard: "bg-white p-10 rounded-[2rem] shadow-xl text-center border border-slate-100",
  loadingTitle: "text-2xl font-heading font-bold text-slate-900",
  loadingText: "mt-3 text-slate-500",
  backToServices: "inline-block mt-6 px-8 py-3 bg-slate-900 text-white rounded-full font-semibold hover:bg-primary-600 transition-all shadow-lg",
};



// Duplicate doctorDetailStyles removed



export const navbarStylesDr = {
  // Main navbar
  navContainer: "fixed top-8 left-1/2 -translate-x-1/2 z-50 w-full md:max-w-2xl lg:max-w-4xl px-4 py-0 rounded-full bg-white/80 backdrop-blur-xl border border-slate-200 shadow-xl flex items-center justify-between gap-3 font-sans transition-all duration-300 hover:shadow-2xl hover:-translate-y-0.5",
  ...
  brandTitle: "text-3xl text-slate-800 font-heading font-bold tracking-wide",
  ...
  activeLink: "bg-slate-900 text-white shadow-lg scale-105 ring-2 ring-slate-200",
  inactiveLink: "text-slate-700 hover:bg-slate-50 hover:text-slate-900 hover:-translate-y-0.5 hover:shadow",
  ...
  logoutButtonDesktop: "hidden lg:flex items-center gap-2 px-4 py-2 rounded-full bg-white text-slate-700 border border-slate-200 shadow-sm text-sm font-semibold transition-all duration-200 transform hover:scale-105 hover:-translate-y-0.5",

  // Hamburger menu buttons
  hamburgerButtonMd: "md:hidden p-2 rounded-md hover:bg-slate-50 transition-colors",
  hamburgerButtonLg: "hidden md:flex lg:hidden p-2 rounded-md hover:bg-slate-50 transition-colors",
  ...
  mobileActiveLink: "bg-slate-50 text-slate-900",
  mobileInactiveLink: "text-slate-700 hover:bg-slate-50",

  // Mobile logout button
  mobileLogoutButton: "mt-2 px-4 py-2 rounded-full text-center bg-slate-900 text-white font-semibold text-sm shadow-sm transition-transform duration-150 hover:scale-105 w-full",
...
export const listPageStyles = {
  // Main container
  pageContainer: "min-h-screen pt-20 md:pt-25 lg:pt-25 font-sans p-4 sm:p-6 bg-white",
  ...
  headerTitle: "text-xl sm:text-xl pt-5 md:pt-0 lg:pt-0 xl:pt-0 font-heading font-extrabold text-slate-900",
  headerSubtitle: "text-xs sm:text-sm text-slate-500",
  ...
  searchIconContainer: "absolute inset-y-0 left-3 flex items-center pointer-events-none text-slate-400",
  searchIcon: "w-4 h-4",
  searchInput: "pl-10 pr-10 w-full sm:w-64 md:w-80 lg:w-96 py-2 rounded-full border border-slate-200 bg-slate-50 text-slate-900 focus:ring-2 focus:ring-primary-500 outline-none font-medium",
  clearSearchButton: "absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-800 p-1 rounded-full",
  clearSearchIcon: "w-4 h-4",
  statusFilter: "text-sm px-3 py-2 rounded-full border border-slate-200 bg-slate-50 w-full sm:w-auto font-medium",

  // Loading and error states
  loadingContainer: "text-center py-8 text-slate-500",
  errorContainer: "text-center py-8 text-rose-500",

  // Appointments grid
  appointmentsGrid: "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 items-start",

  // Appointment card
  appointmentCard: "rounded-2xl p-5 bg-white shadow-sm border border-slate-100 hover:shadow-lg transition-all duration-300 flex flex-col justify-between self-start",

  // Card header
  cardHeader: "flex flex-col sm:flex-row items-start sm:items-center gap-3",
  cardAvatar: "w-12 h-12 sm:w-14 sm:h-14 rounded-full overflow-hidden bg-slate-50 border border-slate-100 flex items-center justify-center",
  cardAvatarImage: "w-full h-full object-cover",
  cardAvatarFallback: "text-slate-700 font-bold",
  cardContent: "flex-1 min-w-0",
  cardPatientName: "text-sm md:text-lg font-bold text-slate-900 truncate",
  cardPatientInfo: "text-xs md:text-sm text-slate-500 mt-1",
  cardDoctorInfo: "mt-2 text-sm text-slate-500 truncate",
  cardDoctorName: "font-semibold text-slate-900",
  cardSpeciality: "text-sm text-slate-600 font-medium truncate",

  // Date and time section
  dateTimeSection: "mt-4 flex flex-col items-start gap-3",
  dateTimeContainer: "text-md md:text-lg text-slate-800 font-bold flex items-center gap-2 w-full",
  calendarIcon: "w-4 h-4",
  dateText: "whitespace-nowrap truncate",
  feeText: "text-sm text-slate-700 font-semibold",

  // Contact and status section
  contactStatusSection: "mt-4 flex flex-col items-start gap-3",
  phoneContainer: "text-sm text-slate-500 flex items-center gap-2",
  phoneIcon: "w-4 h-4",
  ...
  statusSelectEnabled: "bg-white text-slate-800 border-slate-200 hover:shadow-sm",
  statusSelectDisabled: "bg-slate-50 text-slate-400 cursor-not-allowed border-slate-200",

  // Reschedule button container
  rescheduleContainer: "mt-4",
  rescheduleButton: "text-sm px-3 py-1 rounded-full border transition",
  rescheduleButtonEnabled: "bg-white text-slate-800 border-slate-200 hover:shadow-sm",
  rescheduleButtonDisabled: "bg-slate-50 text-slate-400 border-slate-200 cursor-not-allowed",

  // Reschedule form
  rescheduleForm: "flex flex-col md:flex-col items-end gap-2 w-full",
  dateInput: "text-sm px-3 py-2 rounded-full border border-slate-200 bg-white w-full md:w-40",
  timeInput: "text-sm px-3 py-2 rounded-full border border-slate-200 bg-white w-full md:w-36",
  rescheduleButtons: "flex gap-2",
  saveButton: "text-sm px-3 py-2 rounded-full bg-slate-900 text-white font-semibold hover:bg-primary-600 transition",
  cancelButton: "text-sm px-3 py-2 rounded-full border border-slate-200 bg-white text-slate-800 hover:shadow-sm transition"
};
...
export const editProfilePageStyles = {
  // Page container
  pageContainer: "min-h-screen font-sans bg-white p-4 sm:p-5 md:p-6",
  ...
  // Main card
  mainCard: "bg-white rounded-[2.5rem] shadow-xl overflow-hidden border border-slate-100",
  headerBackground: "relative h-24 sm:h-28 md:h-32 bg-gradient-to-r from-slate-800 to-slate-900",
  ...
  profileName: "text-2xl sm:text-3xl md:text-4xl font-heading font-extrabold text-slate-900 truncate",
  profileSubtitle: "text-sm sm:text-lg text-slate-500 mt-2 flex items-center gap-2",
  subtitleIcon: "w-4 h-4",

  // Stats container
  statsContainer: "mt-4 flex flex-wrap items-center gap-3",
  statItem: "flex items-center gap-3 bg-slate-50 px-3 py-2 rounded-full border border-slate-100 shadow-sm text-sm sm:text-base",
  ...
  availabilityToggle: (isAvailable) => `flex items-center gap-3 px-4 sm:px-5 py-2 rounded-full cursor-pointer border-2 shadow-sm transition-all duration-300 ${isAvailable ? "bg-primary-50 border-primary-300 hover:shadow-primary-200" : "bg-slate-50 border-slate-200 hover:shadow-slate-200"} hover:shadow-lg w-full sm:w-auto`,
  toggleTrack: (isAvailable) => `relative w-10 h-5 rounded-full transition-colors ${isAvailable ? "bg-primary-600" : "bg-slate-400"}`,
  ...
  // Form sections
  formSection: "mb-8",
  sectionTitle: "text-xl sm:text-2xl font-heading font-bold text-slate-900 mb-6 flex items-center gap-2",
  sectionIconContainer: "w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center",
  sectionIcon: "w-4 h-4 text-slate-600",

  // Field grid
  fieldGrid: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6",
  fieldGroup: "group",
  fieldHeader: "flex items-center gap-3 mb-2",
  fieldIconContainer: (editing) => `p-2 rounded-full ${editing ? "bg-primary-50 text-primary-600" : "bg-slate-100 text-slate-400"}`,
  fieldIcon: "w-4 h-4",
  fieldLabel: "text-sm font-semibold text-slate-700",

  // Input fields
  inputBase: (editing) => `w-full rounded-2xl border-2 px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base transition-all duration-200 ${editing ? "border-slate-200 bg-slate-50 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 focus:bg-white" : "border-slate-100 bg-slate-50 text-slate-500 cursor-not-allowed"}`,
  // About textarea
  aboutTextarea: (editing) => `w-full rounded-xl border-2 px-4 py-3 text-sm sm:text-base transition-all duration-200 ${editing ? "border-slate-200 bg-slate-50 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 focus:bg-white" : "border-slate-100 bg-slate-50 text-slate-500 cursor-not-allowed"}`,
  aboutCharCount: "absolute bottom-3 right-3 text-xs text-slate-400",

  // Schedule section
  scheduleHeader: "flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6",
  emptySchedule: "text-center py-10 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50",
  emptyScheduleIcon: "w-12 h-12 text-slate-300 mx-auto mb-3",
  emptyScheduleText: "text-slate-600 font-medium",
  emptyScheduleSubtext: "text-sm text-slate-400 mt-1",

  // Schedule grid
  scheduleGrid: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5",

  // Date card
  dateCard: "group relative bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1",
  dateHeader: "flex items-center justify-between mb-3 pb-3 border-b border-slate-100",
  dateIconContainer: "p-2 rounded-full bg-slate-100",
  dateIcon: "w-5 h-5 text-slate-600",
  dateTitle: "font-bold text-base sm:text-lg text-slate-900",
  dateSubtitle: "text-xs sm:text-sm text-slate-500",
  dateSlotCount: "text-xs font-medium px-3 py-1 rounded-full bg-slate-100 text-slate-700",
  dateDeleteButton: (editing) => `p-2 rounded-full cursor-pointer transition-colors ${editing ? "hover:bg-rose-50 text-rose-500 hover:text-rose-600" : "text-slate-300 cursor-not-allowed"}`,
  dateDeleteIcon: "w-4 h-4",

  // Time slots
  timeSlotContainer: "space-y-3",
  timeSlotItem: "flex items-center justify-between bg-slate-50 px-3 py-2 rounded-full border border-slate-100 hover:border-slate-200 transition-colors",
  timeSlotIcon: "w-4 h-4 text-slate-500",
  timeSlotText: "font-medium text-slate-900 text-sm sm:text-base",
  timeSlotDeleteButton: (editing) => `p-1.5 rounded-full cursor-pointer transition-colors ${editing ? "hover:bg-rose-50 text-rose-500 hover:text-rose-600" : "text-slate-300 cursor-not-allowed"}`,
  timeSlotDeleteIcon: "w-4 h-4",

  // Add time slot
  addSlotContainer: "pt-3 border-t border-slate-100",
  addSlotInput: "flex-1 rounded-full px-3 py-2 text-sm border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-primary-200",
  addSlotButton: "p-2 rounded-full cursor-pointer bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors",
  addSlotIcon: "w-4 h-4",

  // Save message
  saveMessage: (type) => `px-4 py-2 rounded-lg ${type === "saving" ? "bg-blue-50 text-blue-700 border border-blue-200" : "bg-primary-50 text-primary-700 border border-primary-200"}`,

  // Actions section
  actionsSection: "flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-slate-100",
  actionsText: "text-sm text-slate-400",
  actionsButtons: "flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto",
  resetButton: "px-6 py-3 rounded-full cursor-pointer border-2 border-slate-200 text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 font-medium w-full sm:w-auto text-center",
  saveButton: "group relative overflow-hidden bg-slate-900 hover:bg-primary-600 text-white px-6 py-3 rounded-full cursor-pointer shadow-lg transition-all duration-300 hover:shadow-xl disabled:opacity-70 disabled:cursor-not-allowed w-full sm:w-auto text-center",
  ...
  addDateInput: "rounded-xl px-3 py-2 border-2 border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-400 text-sm sm:text-base",
  addDateButton: "flex items-center gap-2 bg-slate-900 hover:bg-primary-600 text-white px-4 py-2 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 text-sm sm:text-base",
  addDateIcon: "w-4 h-4",
...
export const dashboardStyles = {
  // Main container
  pageContainer: "min-h-screen font-sans pt-16 lg:pt-20 md:pt-15 p-4 sm:p-6 bg-white",
  ...
  // Header section
  headerContainer: "mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4",
  headerTitle: "text-2xl pt-10 xl:pt-0 uppercase lg:pt-0 sm:text-3xl font-heading font-extrabold tracking-tight text-slate-900",
  headerSubtitle: "text-sm sm:text-base text-slate-500",
  headerInfo: "text-sm text-slate-500",
  refreshButton: "text-sm px-3 py-1 rounded-full bg-white text-slate-700 border border-slate-200 hover:shadow-sm font-medium",

  // Stats grid
  statsGrid: "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8",

  // Stat card
  statCard: "rounded-2xl p-4 bg-white border border-slate-100 shadow-sm hover:shadow-lg transition-all duration-300",
  statContent: "flex items-center justify-between gap-4",
  statTextContainer: "flex-1",
  statTitle: "text-sm font-medium text-slate-500",
  statValue: "mt-2 text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight",
  statIconContainer: "p-3 rounded-full bg-gradient-to-br border shadow-md",
  statIcon: "w-6 h-6 text-slate-900",
  ...
  appointmentsContainer: "bg-white rounded-2xl p-5 border border-slate-100 shadow-sm",
  appointmentsHeader: "flex items-center justify-between mb-4",
  appointmentsTitle: "text-lg sm:text-xl font-heading font-bold text-slate-900",
  appointmentsTotal: "text-sm sm:text-base text-slate-500 flex items-center gap-2",
  totalIcon: "w-4 h-4",
  ...
  // Appointment card
  appointmentCard: "rounded-2xl p-5 bg-white shadow-sm border border-slate-100 flex flex-col justify-between gap-4 hover:shadow-lg transition-all duration-300 self-start",
  cardHeader: "flex items-start gap-3",
  cardAvatar: "w-12 h-12 sm:w-14 sm:h-14 rounded-full overflow-hidden bg-slate-50 border border-slate-100 flex items-center justify-center",
  cardAvatarImage: "w-full h-full object-cover",
  cardAvatarFallback: "text-slate-700 font-bold",
  cardContent: "flex-1",
  cardPatientName: "text-base sm:text-lg font-bold text-slate-900",
  cardPatientInfo: "text-xs sm:text-sm text-slate-500 mt-1",
  cardDoctorInfo: "mt-2 text-sm sm:text-sm text-slate-500",
  cardDoctorName: "font-semibold text-slate-900",
  cardSpeciality: "text-xs sm:text-sm text-slate-600 font-medium",
  cardPhoneContainer: "mt-2 text-xs sm:text-sm text-slate-500 flex items-center gap-2",
  cardPhoneIcon: "w-4 h-4",

  // Date and time section
  dateTimeContainer: "flex items-center justify-between",
  dateText: "text-sm sm:text-lg font-bold text-slate-800",
  timeText: "text-sm sm:text-base font-semibold text-slate-900",

  // Card footer
  cardFooter: "flex flex-col items-end gap-2",
  feeText: "text-sm sm:text-base text-slate-700 font-medium",
  statusContainer: "flex items-center gap-2",

  // Show more button
  showMoreContainer: "mt-4 flex justify-center",
  showMoreButton: "px-4 py-2 rounded-full bg-slate-900 text-white text-sm font-semibold hover:bg-primary-600 transition shadow-lg",
  ...
  rescheduleDateInput: "text-xs px-3 py-2 rounded-full border border-slate-200 bg-white w-full md:w-48 lg:w-56",
  rescheduleTimeInput: "text-xs px-3 py-2 rounded-full border border-slate-200 bg-white w-full md:w-48 lg:w-56",
  rescheduleButtons: "flex gap-2",
  saveButton: "text-xs px-3 py-2 rounded-full bg-slate-900 text-white font-semibold hover:bg-primary-600 transition",
  cancelButton: "text-xs px-3 py-2 rounded-full border border-slate-200 bg-white text-slate-800 hover:shadow-sm transition"
};

export const doctorDetailStyles = {
  // Page States
  pageContainer: "min-h-screen font-serif bg-linear-to-br from-emerald-50 via-white to-teal-50",
  loadingContainer: "min-h-screen flex items-center justify-center text-emerald-600 font-semibold text-lg bg-emerald-50",
  errorContainer: "min-h-screen flex items-center justify-center bg-rose-50 p-6",
  errorContent: "bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-rose-100",
  errorText: "text-rose-600 font-bold mb-2 text-xl",
  errorMessage: "text-gray-600 mb-6 font-medium",
  notFoundContainer: "min-h-screen flex items-center justify-center bg-gray-50 p-6",
  notFoundContent: "bg-white p-10 rounded-2xl shadow-xl max-w-md w-full text-center flex flex-col items-center",
  notFoundEmoji: "text-6xl mb-4",
  notFoundTitle: "text-2xl font-bold text-gray-800 mb-6",
  backButton: "inline-flex items-center gap-2 bg-emerald-100 text-emerald-700 px-6 py-2.5 rounded-full font-semibold hover:bg-emerald-200 transition-colors duration-300",

  // Header 
  headerContainer: "sticky top-0 z-40 bg-white/80 backdrop-blur-lg border-b border-emerald-100 shadow-sm",
  headerContent: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20",
  headerFlex: "flex items-center justify-between h-full",
  headerBackButton: "flex items-center gap-2 text-emerald-700 hover:text-emerald-900 transition-colors font-medium group cursor-pointer",
  headerBackButtonText: "hidden sm:inline font-semibold group-hover:-translate-x-1 transition-transform",
  headerTitle: "text-xl sm:text-2xl font-bold bg-clip-text text-transparent bg-linear-to-r from-emerald-700 to-teal-600 tracking-tight",
  headerRatingContainer: "flex items-center gap-1.5 bg-yellow-50 px-3 py-1.5 rounded-full border border-yellow-200 shadow-sm",
  headerRatingIcon: "text-yellow-500 fill-yellow-500",
  headerRatingText: "font-bold text-yellow-700 text-sm",

  // Content Layout
  mainContent: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 transition-all duration-700 transform relative z-10",
  visibleState: "opacity-100 translate-y-0",
  hiddenState: "opacity-0 translate-y-8 absolute pointer-events-none -z-10",

  // Profile Card
  profileCard: "bg-white/90 backdrop-blur-md rounded-3xl shadow-xl border border-emerald-100 overflow-hidden mb-10 transition-all hover:shadow-2xl hover:border-emerald-200 p-1",
  profileGrid: "flex flex-col lg:flex-row bg-white p-6 sm:p-8 lg:p-10 rounded-[22px]",

  // Left Column (Avatar + Stats)
  leftColumn: "lg:w-1/3 flex flex-col items-center border-b lg:border-b-0 lg:border-r border-emerald-100 pb-8 lg:pb-0 lg:pr-10 mb-8 lg:mb-0",
  avatarContainer: "relative w-40 h-40 sm:w-48 sm:h-48 mb-6 group cursor-pointer",
  avatarGlow: "absolute inset-0 bg-linear-to-tr from-emerald-400 to-teal-400 rounded-full blur-xl opacity-30 group-hover:opacity-50 transition-opacity duration-300",
  avatarImage: "relative w-full h-full rounded-full object-cover border-4 border-white shadow-2xl z-10",

  // Stats
  statsGrid: "grid grid-cols-3 gap-3 w-full max-w-sm",
  statBox: "flex flex-col items-center justify-center p-3 rounded-2xl bg-emerald-50/50 border border-emerald-100 hover:bg-emerald-50 transition-colors duration-300",
  statIcon: "w-5 h-5 mb-2",
  heartIcon: "text-rose-500 fill-rose-100",
  awardIcon: "text-amber-500",
  usersIcon: "text-blue-500",
  statValue: "font-bold text-gray-900 text-sm sm:text-base mb-0.5",
  statLabel: "text-xs font-medium text-gray-500",

  // Right Column (Details)
  rightColumn: "lg:w-2/3 lg:pl-10 flex flex-col justify-center space-y-8",
  doctorName: "text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight",
  specializationBadge: "inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-100 text-emerald-800 rounded-full font-semibold text-sm",
  badgeIcon: "w-4 h-4 text-emerald-600 fill-emerald-600",

  // Info Grid
  infoGrid: "grid sm:grid-cols-2 gap-4 lg:gap-6",
  infoItem: "flex items-start gap-3.5 p-4 rounded-2xl hover:bg-emerald-50/50 transition-colors border border-transparent hover:border-emerald-100",
  infoIcon: "w-5 h-5 mt-0.5 text-emerald-600",
  infoLabel: "text-xs font-bold text-emerald-800 uppercase tracking-wider mb-1",
  infoValue: "text-gray-700 font-medium",
  feeValue: "text-lg font-bold text-emerald-700",

  // About Section
  aboutContainer: "bg-gray-50/80 rounded-2xl p-5 border border-gray-100 relative mt-4",
  aboutHeader: "flex items-center gap-2 mb-3",
  aboutIcon: "w-4 h-4 text-emerald-600",
  aboutTitle: "text-base font-bold text-gray-900",
  aboutText: "text-gray-600 leading-relaxed font-medium text-sm sm:text-base",

  // Appointment Section
  appointmentContainer: "bg-white rounded-3xl shadow-xl border border-emerald-100 relative z-20 mt-8",
  appointmentContent: "p-6 sm:p-8 lg:p-10",
  appointmentHeader: "flex items-center gap-3 mb-8 pb-6 border-b border-emerald-100",
  appointmentIcon: "w-6 h-6 sm:w-8 sm:h-8 text-emerald-600",
  appointmentTitle: "text-2xl sm:text-3xl font-bold bg-clip-text text-transparent bg-linear-to-r from-emerald-700 to-teal-600 tracking-tight",

  appointmentGrid: "grid lg:grid-cols-2 gap-8 lg:gap-12",

  // Date Section
  dateSection: "space-y-6 flex flex-col",
  dateTitle: "flex items-center gap-2 text-lg font-bold text-gray-900",
  dateTitleIcon: "w-5 h-5 text-emerald-600",

  dateScrollContainer: "relative w-full z-30",
  dateButtonsContainer: "flex overflow-x-auto overflow-y-hidden gap-3 pb-4 pt-1 px-1 snap-x no-scrollbar pointer-events-auto",
  dateButton: "flex-shrink-0 snap-center transition-all duration-300 rounded-2xl outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-2 w-[72px] sm:w-[84px] cursor-pointer relative z-40",
  dateButtonSelected: "bg-linear-to-b from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-500/30 transform scale-105 border-0",
  dateButtonUnselected: "bg-white text-gray-600 border border-emerald-200 hover:border-emerald-400 hover:bg-emerald-50",
  dateContent: "flex flex-col items-center py-3.5 sm:py-4 gap-1 transform transition-transform group-hover:scale-105",
  dateWeekday: "text-xs font-semibold uppercase tracking-wider opacity-90",
  dateDay: "text-2xl sm:text-3xl font-black leading-none",
  dateMonth: "text-xs font-bold uppercase opacity-90",

  // Form Section
  patientForm: "flex-1 min-h-[300px] flex flex-col pt-6 lg:pt-2",
  patientFormTitle: "text-lg font-bold text-gray-900 mb-4 inline-flex items-center pb-2 border-b-2 border-emerald-100",
  patientFormGrid: "space-y-4",
  formInput: "w-full bg-emerald-50/50 border border-emerald-200 text-gray-800 rounded-xl px-4 py-3 sm:py-3.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all placeholder-gray-400 font-medium hover:bg-white",
  formSelect: "w-full bg-emerald-50/50 border border-emerald-200 text-gray-800 rounded-xl px-4 py-3 sm:py-3.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all appearance-none font-medium cursor-pointer hover:bg-white",
  emailInput: "w-full bg-emerald-50/50 border border-emerald-200 text-gray-800 rounded-xl px-4 py-3 sm:py-3.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all placeholder-gray-400 font-medium hover:bg-white",

  // Time Slots Section
  timeSlotsSection: "flex flex-col bg-gray-50/50 rounded-3xl p-6 sm:p-8 border border-emerald-100 h-full",
  timeSlotsTitle: "flex items-center gap-2 text-lg font-bold text-gray-900 mb-6",
  timeSlotsIcon: "w-5 h-5 text-emerald-600",
  timeSlotsContainer: "grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-3 mb-8 auto-rows-max overflow-y-auto max-h-[160px] sm:max-h-[220px] pr-2 no-scrollbar pointer-events-auto z-30 relative",
  noSlotsMessage: "col-span-full py-6 text-center text-sm font-medium text-gray-500 bg-white border border-dashed border-emerald-200 rounded-xl",
  timeSlotButton: "py-2.5 sm:py-3 cursor-pointer rounded-xl border font-bold transition-all duration-300 outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-1 relative z-40 pointer-events-auto",
  timeSlotButtonSelected: "bg-emerald-600 text-white border-emerald-600 shadow-md shadow-emerald-500/20 transform scale-[1.02]",
  timeSlotButtonUnselected: "bg-white text-emerald-700 border-emerald-200 hover:border-emerald-500 hover:bg-emerald-50 transform hover:scale-[1.02]",
  timeSlotContent: "flex items-center justify-center text-sm sm:text-[15px] tracking-tight",
  timeSlotIcon: "w-4 h-4 mr-1.5 opacity-80",

  // Summary and Booking
  summaryContainer: "mt-auto bg-white rounded-2xl border border-emerald-100 p-5 sm:p-6 shadow-sm",
  summaryItem: "space-y-3 mb-6 bg-gray-50 p-4 rounded-xl",
  summaryRow: "flex justify-between items-center text-sm sm:text-base border-b border-gray-200/50 pb-2 last:border-0 last:pb-0",
  summaryLabel: "font-medium text-gray-600",
  summaryValue: "font-bold text-gray-900 text-right max-w-[60%] truncate",
  feeDisplay: "font-black text-emerald-700 text-lg flex items-center gap-1",

  // Payment Selection
  paymentContainer: "mb-6",
  paymentLabel: "block text-sm font-bold text-gray-800 mb-3 uppercase tracking-wider",
  paymentOptions: "flex gap-3",
  paymentOption: "flex-1 flex items-center justify-center p-3 sm:p-4 rounded-xl border-2 text-sm md:text-[15px] font-bold cursor-pointer transition-all duration-300",
  paymentOptionSelected: "bg-emerald-50 border-emerald-500 text-emerald-800 shadow-inner",
  paymentOptionUnselected: "bg-white border-gray-100 text-gray-600 hover:border-emerald-300 hover:bg-gray-50",
  paymentRadio: "hidden",

  // Booking Button
  bookingButton: "w-full cursor-pointer rounded-xl py-4 transition-all duration-300 font-bold outline-none focus:ring-4 text-base focus:ring-emerald-500/50 shadow-xl overflow-hidden relative group",
  bookingButtonEnabled: "bg-linear-to-r from-emerald-600 to-teal-600 text-white hover:shadow-emerald-500/40 hover:-translate-y-0.5",
  bookingButtonDisabled: "bg-gray-300 text-gray-500 cursor-not-allowed shadow-none",
  bookingButtonContent: "flex items-center justify-center gap-2 relative z-10",
  bookingIcon: "w-5 h-5",
};