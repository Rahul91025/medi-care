...
export default function DoctorDetail() {
  ...
  const next7 = useMemo(() => {
    let dates = getScheduleDates(doctor?.schedule);

    // Fallback: If no dates are in the schedule, generate the next 7 days automatically
    if (dates.length === 0 && doctor) {
      const today = new Date();
  ...
  const slots = useMemo(() => {
    ...
    const existingSlots = doctor.schedule && doctor.schedule[key];

    // Fallback: Default time slots if the database does not have them defined yet
    if (!existingSlots || existingSlots.length === 0) {
      return ["09:00 AM", "10:30 AM", "01:00 PM", "03:30 PM", "05:00 PM", "06:30 PM"];
    }
